...
void Shape::setPixel(int x, int y, Uint32 color) {
    Color c = Color();
...
void Shape::setPixel(int x, int y, Color color) {
	this->setPixel(x, y, color.getR(),color.getG(),color.getB());
}

void Shape::setPixel(int x, int y, int r, int g, int b) {
    setPixel(x, y, r, g, b, 255);
}

void Shape::setPixel(int x, int y, int r, int g, int b, int a) {
    unsigned int * pixels;
...
Uint32 Shape::getPixel(int x, int y)
{
    SDL_Surface * window_surface = Context::getInstance()->getWindowSurface();
